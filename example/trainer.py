...
def train():
    ...
    print('-'*30)
    model = get_unet()
    model_checkpoint = ModelCheckpoint('pract_weights.h5', monitor='val_loss', save_best_only=True)
    ...
    csv_logger = CSVLogger('pract_training.log')
    # es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=50)

    # Fitting the model
    history = model.fit(imgs_train, imgs_mask_train, batch_size=4, nb_epoch=200, verbose=1, shuffle=True,
    ...
    # summarize history for training dice coef
